...
public class DES_2DES_MITM {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.printf("alegem biti random: k1Byte=0x%02X, k2Byte=0x%02X ", seed1 & 0xFF, seed2 & 0xFF);
                System.out.println("Plaintext-ul (hex):  " + bytesToHex(plaintext));

        byte[] ciphertext = doubleDesEncryptPure(k1, k2, plaintext);
        System.out.println("Cifru 2DES: " + bytesToHex(ciphertext));

        Map<Integer,Integer> candidates = mitmAttackPure(plaintext, ciphertext);

        System.out.println("--- verificarea candidatilor ---");
        ...
        for(Map.Entry<Integer,Integer> e : candidates.entrySet()){
            ...
            System.out.printf("candidatul k1_par=0x%02X k2_par=0x%02X  -> reproduc cifrul? %s  -> reproduce textul dupa decriptare? %s ", k1p, k2p, cEq?"YES":"NO", pEq?"YES":"NO");
            if(cEq && pEq && k1p == (k1[0] & 0xFF) && k2p == (k2[0] & 0xFF)) confirmed = true;
        }

        System.out.println("MITM candidates (parity-fixed k1 -> parity-fixed k2) found: " + candidates.size());
        ...
        if(!candidates.isEmpty()){
            ...
            System.out.println("Plaintext inaninte de criptare (ASCII): " + new String(plaintext, "ASCII") + "  hex=" + bytesToHex(plaintext));
            System.out.println("Plaintext dupa criptare cu primul candidat (ASCII): " + new String(decrypted, "ASCII") + "  hex=" + bytesToHex(decrypted));
        }
    }
}
