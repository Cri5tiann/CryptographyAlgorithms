...
public class DES_lab2_Simplu {

    // --- Tabele --- (IP, FP, E, S-boxes, P, PC1, PC2, SHIFTS)
    ...
    private static byte[] desBlockWithSubkeys(byte[] block8, long[] subkeys){ long block = bytesToLong(block8); long ip=0L; for(int i=0;i<64;i++){ int bit=getBit64(block, IP[i]); if(bit==1) ip |= (1L << (63 - i)); } int L=(int)(ip>>>32); int R=(int)(ip & 0xFFFFFFFFL); for(int round=0; round<16; round++){ long expanded = expand32to48(R); long x = expanded ^ subkeys[round]; int sOut = sBoxes(x); int fOut = permuteP(sOut); int newR = L ^ fOut; L = R; R = newR; } long preout = (((long)R) & 0xFFFFFFFFL) << 32 | (((long)L) & 0xFFFFFFFFL); long fp=0L; for(int i=0;i<64;i++){ int bit=getBit64(preout, FP[i]); if(bit==1) fp |= (1L << (63 - i)); } return longToBytes(fp); }
    ...
    public static void main(String[] args) throws Exception{
        ...
        int seedInt;
        if(args.length>=1){ String s=args[0].trim(); if(s.startsWith("0x")||s.startsWith("0X")) seedInt=Integer.parseInt(s.substring(2),16); else seedInt=Integer.parseInt(s); }
        else seedInt=rnd.nextInt(10); // numara random 0..9
        byte seed=(byte)(seedInt & 0xFF);

        byte[] key = makeDESKeyFromByte(seed);
        byte[] plaintext = "TESTDES1".getBytes(StandardCharsets.US_ASCII); // 8 bytes

        System.out.printf("Byte-ul seed (raw): 0x%02X ", seed & 0xFF);
                System.out.println("Parity-fixed DES key (8 bytes hex): " + bytesToHex(key));
        System.out.println("Plaintextul (ASCII): " + new String(plaintext, StandardCharsets.US_ASCII));
        System.out.println("Plaintextul (hex)  : " + bytesToHex(plaintext));

        byte[] cipher = desEncryptBlockPure(key, plaintext);
        System.out.println("Ciphertextul (hex) : " + bytesToHex(cipher));

        byte[] recovered = desDecryptBlockPure(key, cipher);
        System.out.println("Recuperat (hex)  : " + bytesToHex(recovered));
        System.out.println("recuperat (ASCII): " + new String(recovered, StandardCharsets.US_ASCII));

        System.out.println("criptare reusita? " + (Arrays.equals(plaintext, recovered) ? "DA" : "NU"));
    }
}
