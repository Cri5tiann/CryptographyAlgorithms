...
public class DES {
    ...
    public static void main(String[] args) throws Exception{
        ...
        System.out.printf("Seed byte (raw): 0x%02X ", seed & 0xFF);
                System.out.println("Parity-fixed DES key (8 bytes hex): " + bytesToHex(key));
        System.out.println("Plaintext (ASCII): " + new String(plaintext, StandardCharsets.US_ASCII));
        System.out.println("Plaintext (hex)  : " + bytesToHex(plaintext));

        byte[] cipher = desEncryptBlockPure(key, plaintext);
        System.out.println("Ciphertext (hex) : " + bytesToHex(cipher));

        byte[] recovered = desDecryptBlockPure(key, cipher);
        System.out.println("Recovered (hex)  : " + bytesToHex(recovered));
        System.out.println("Recovered (ASCII): " + new String(recovered, StandardCharsets.US_ASCII));

        System.out.println("successfull decription? " + (Arrays.equals(plaintext, recovered) ? "DA" : "NU"));
    }
}
